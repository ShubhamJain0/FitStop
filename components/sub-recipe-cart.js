...
export default function SubRecipeCart({ navigation }) {
  ...
  const [phone, setPhone] = useState('');
    ...
    useEffect(() => {
            (async () => {
                ...
                if (token) {
                    fetch('http://192.168.0.156:8000/store/recipesubsconfirm/',{
                        method: 'GET',
                        headers: {
                    ...
                    .then(resp => {if (mounted) {setCartList(resp.json.items), setTotal(resp.json.total), setCartStatus(resp.status), setItemPhotos(resp.json.photos)}})
...
const createPaymentOrder = async (payMethod) => {
  ...
  const openRazorpayCheckout = async (order_id, payMethod) => {
    ...
    RazorpayCheckout.open(options).then((data) => {
      // handle success
        ...
        if (token) {
            fetch('http://192.168.0.156:8000/store/recipesubscribe/',{
                method: 'POST',
                headers: {
                'Authorization': `Token ${token}`,
                'Content-type': 'application/json'
                ...
                body: JSON.stringify({pushToken: conPushToken === null ? null: conPushToken.data, total_price: appliedCoupon ? (total * subMultiply) + deliveryCharges + taxes - appliedCoupon.discount: (total * subMultiply) + deliveryCharges + taxes, 
                    ...
                    plan: choosenPlan, startdate: choosenDay.dateString, enddate: choosenPlan === '2-week' ? moment(choosenDay.dateString).add(13, 'days').format('YYYY-MM-DD') : moment(choosenDay.dateString).add(1, 'month').format('YYYY-MM-DD')})
            })
            .then(resp => resp.json().then(data => ({status: resp.status, json: data})))
            .then(resp => {
                if (resp.status === 201) {
  ...
  const deleteCart = () => async evt=>  {
    ...
    const deleteItem = async (item) => {
        const token = await SecureStore.getItemAsync('USER_TOKEN')
        if (token)
            fetch('http://192.168.0.156:8000/store/recipesubsreduceordelete/',{
                method: 'DELETE',
                headers: {
                'Authorization': `Token ${token}`,
                'Content-type': 'application/json'
      ...
      <View style={styles.container}>
        ...
        <ScrollView bounces={false} showsVerticalScrollIndicator={false} contentContainerStyle={{paddingBottom: 50, paddingTop: 15}} overScrollMode={'never'}>
            ...
            <View>
                ...
                {cartStatus === 200 ? cartList.map((item) => {
                    return (
                        <View style={{backgroundColor: 'white', flex: cartList.length > 1 ? 1 : 0, elevation: 5, shadowOffset: {width: 0, height: 2}, shadowRadius: 3.84, shadowOpacity: 0.25, shadowColor: '#000', marginBottom: 15, marginRight: 35, marginTop: 10, padding: 20, paddingBottom: 60, borderRadius: 10}} key={item.id}>
                            {itemPhotos.map((item1) => {
                                return item1.name === item.recipe_name ? 
                                <View key={item1.id}>
                                    <Image source={{uri: item1.image}} style={{width: 80, height: 80, alignSelf: 'center'}} />
                                </View>
                                : null
                            })}
                            <Text style={{fontFamily: 'Maven-sem', textAlign: 'center', marginTop: 5, fontSize: wp(4), color: '#249c86'}}>{item.category}</Text>
                            <Text style={{fontFamily: 'Maven-sem', textAlign: 'center', marginTop: 5, fontSize: wp(4), color: 'black'}}>{item.recipe_name}</Text>
                            ...
                            <Text style={{fontFamily: 'Maven-sem', fontSize: wp(3.5), position: 'absolute', bottom: 5, alignSelf: 'center', color: 'black'}}> &#8377; {item.items_price['price__sum']} </Text>
            ...
            <View style={{flex: 1, marginTop: 20, width: '85%', alignSelf: 'center'}}>
                <View style={{marginBottom: 25}}>
                    ...
                    <Text style={{fontFamily: 'Maven-med', fontSize: wp(3.5), marginTop: 5, marginLeft: 25}}>{choosenPlan} plan. Starting from {choosenDay.dateString} and ends on {choosenPlan === '2-week' ? moment(choosenDay.dateString).add(13, 'days').format('YYYY-MM-DD') : moment(choosenDay.dateString).add(1, 'month').format('YYYY-MM-DD')}</Text>
                ...
                <View style={{marginTop: 40, borderRadius: 10, marginBottom: 50}}>
                    ...
                    <View style={{flexDirection: 'row', marginBottom: 10, alignItems: 'center'}}>
                        <Text style={{flex: 1, fontFamily: 'Maven-sem', fontSize: wp(3.5), color: 'grey'}}>For {choosenPlan === '2-week' ? '14 deliveries (2-week plan)': '31 deliveries (month plan)'}</Text>
        ...
        <View style={{position: 'absolute', backgroundColor: '#fcfcfc', padding: 15, bottom: 0, width: '100%', flexDirection: 'row', alignItems: 'center', elevation: 20, shadowColor: "#000", shadowOffset: {width: 0, height: 10}, shadowOpacity: 0.51, shadowRadius: 13.16}}>
            ...
            {onPlace ? <ActivityIndicator color="#6aab9e" size={50} style={{flex: 1}} />
                ...
                <Ripple onPress={() => setPaymentModal(true)} style={{flex: 1, opacity: 1, backgroundColor: '#6aab9e', borderRadius: 5, padding: 15, alignSelf: 'center', width: '60%', elevation: 5, shadowOffset: {width: 0, height: 2}, shadowOpacity: 0.25, shadowRadius: 3.84, shadowColor: '#000'}} activeOpacity={1}>
                    <Text style={{textAlign: 'center', fontFamily: 'Maven-sem', fontSize: wp(4), color: 'black'}}>Pay Now &raquo;</Text>
                </Ripple>:
                <TouchableOpacity disabled={true} style={{flex: 1, opacity: 0.1, backgroundColor: '#6aab9e', borderRadius: 5, padding: 15, alignSelf: 'center', width: '60%'}} activeOpacity={1}>
