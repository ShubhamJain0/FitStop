...
import { StyleSheet, Text, View, TouchableOpacity, TextInput, Image, ScrollView, Platform, ActivityIndicator } from 'react-native';
...
export default function PreviousOrders({ navigation }) {
    ...
    const [choose, setChoose] = useState('Previous');
    ...
    useEffect(() => {
        (async () => {
            ...
            if (token) {
              ...
              .then(resp => {if (mounted) {setList(resp.json.qs), setItems(resp.json.data), setStatus(resp.status)}})
    ...
    return (
        <View style={styles.container}>
            <View style={{flexDirection: 'row', alignItems: 'center', marginBottom: 30, width: '85%', alignSelf: 'center'}}>
                <Text style={{backgroundColor: '#99b898', flex: 1, borderRadius: 5, padding: 10, textAlign: 'center', fontFamily: 'sf-semi', fontSize: wp(4)}}>Previous Orders</Text>
                <TouchableOpacity style={{flex: 1, padding: 10}} onPress={() => setChoose('Active')} >
                    <Text style={{fontFamily: 'sf-semi', fontSize: wp(4), textAlign: 'center'}}>Active Orders</Text>
                </TouchableOpacity>
            </View>
            <ScrollView bounces={false} showsVerticalScrollIndicator={false} contentContainerStyle={{paddingTop: 15}}>
                {status === 200 ? list.map(item => {
                    return(
                        <View key={item.id} style={{marginBottom: 50, backgroundColor: 'white', width: '85%', padding: 25, paddingTop: 15, paddingBottom: 15, borderRadius: 10, alignSelf: 'center', elevation: 10, shadowOffset: {width: 0, height: 5}, shadowOpacity: 0.34, shadowRadius: 6.27}}>
    ...
    return (
        <View style={styles.container}>
            <View style={{flexDirection: 'row', alignItems: 'center', width: '85%', alignSelf: 'center'}}>
                <TouchableOpacity style={{flex: 1, padding: 10}} onPress={() => setChoose('Previous')} >
                    <Text style={{fontFamily: 'sf-semi', fontSize: wp(4), textAlign: 'center'}}>Previous Orders</Text>
                </TouchableOpacity>
                <Text style={{flex: 1, textAlign: 'center', borderRadius: 5, padding: 10, backgroundColor: '#99b898', fontFamily: 'sf-semi', fontSize: wp(4)}}>Active Orders</Text>
            </View>
            {activeOrders ? <Text style={{fontFamily: 'sofia-black', width: '85%', alignSelf: 'center', fontSize: wp(5), marginTop: 25, color: '#99b898', paddingBottom: 25}}>You have {activeOrders.length} active order(s)</Text>: null}
                    ...
                    activeOrders.map((item) => {
                        return (
                            <View key={item.id} style={{width: '85%', alignSelf: 'center', marginTop: 25, backgroundColor: 'white', elevation: 10, shadowOffset: {width: 0, height: 5}, shadowOpacity: 0.34, shadowRadius: 6.27, padding: 25, borderRadius: 20}} >
                                ...
                                <Text style={{backgroundColor: '#cccccc', height: 1, marginTop: 25}}></Text>
                                <View style={{flexDirection: 'row', alignItems: 'center',  marginTop: 15}}>
