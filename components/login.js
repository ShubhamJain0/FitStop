...
export default function Auth({ navigation }) {
    ...
    const sendOTP = () => {
        setDisabled(true);
        fetch('http://192.168.0.107:8000/api/send_sms_code/',{
          method: 'POST',
          headers: {
        ...
        .then(resp => {if (resp.status === 200){
            ...
            alert("No account associated with this number. If you haven't registered yet, please register now!");
            setDisabled(false);
        }})
    ...
    const Login = () => {
        ...
        fetch('http://192.168.0.107:8000/api/customauth/',{
          method: 'POST',
          headers: {
            'Content-type': 'application/json'
        },
        ...
        .then(resp =>  resp.json().then(data => ({status: resp.status, json: data})))
        .then(resp => {return (resp.json.token ? (saveToken(resp.json.token), navigation.goBack()): alert('OTP did not match or has expired'), setVerifyOTPDisabled(false))})
        .catch(error => console.log(error))
    }
        ...
        <View style={{backgroundColor: 'white', width: '100%', height: '100%', position: 'absolute', top: hp(45), paddingTop: wp(8), paddingLeft: wp(15), alignSelf: 'center', borderRadius: 50, elevation: 25, shadowOffset: {width: 0.5, height: 2}, shadowRadius: 5, shadowOpacity: 0.3}}>
            ...
            <Modal isVisible={otpModal} >
                ...
                {OTP.toString().length < 6 ? 
                    ...
                    <TouchableOpacity disabled={verifyOTPDisabled} style={verifyOTPDisabled ? {opacity: 0.2}: {opacity: 1}} onPress={Login}>
                        <Text>Verify OTP</Text>
                    </TouchableOpacity>
                }
